...
import { UndoIcon, RedoIcon, EyeIcon, ExpandIcon } from './components/icons';
...
const App: React.FC = () => {
  ...
  const addImageToHistory = useCallback((newImageFile: File) => {
    ...
    setIsLowRes(false); // Any edit results in a new image, so reset the flag
  ...
  const handleImageUpload = useCallback((file: File) => {
    ...
    reader.onload = (e) => {
        const img = new Image();
        img.onload = () => {
            const isLow = img.naturalWidth < 1024 || img.naturalHeight < 1024;
            setIsLowRes(isLow);
            setActiveTab(isLow ? 'upscale' : 'retouch');
            setHistory([file]);
            setHistoryIndex(0);
        };
  ...
  const handleApplyUpscale = useCallback(async () => {
    ...
    try {
        ...
        addImageToHistory(newImageFile);
        setActiveTab('retouch'); // Switch to retouch after upscaling for further edits
    } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-sm">
            {(['retouch', 'upscale', 'crop', 'adjust', 'filters'] as Tab[]).map(tab => {
                const isUpscaleTab = tab === 'upscale';
                const showUpscaleGlow = isUpscaleTab && isLowRes && !isLoading;
                return (
                 <button
                    key={tab}
                    onClick={() => setActiveTab(tab)}
                    className={`w-full capitalize font-semibold py-3 px-5 rounded-md transition-all duration-200 text-base flex items-center justify-center ${
                        activeTab === tab 
                        ? 'bg-gradient-to-br from-blue-500 to-cyan-400 text-white shadow-lg shadow-cyan-500/40' 
                        : showUpscaleGlow
                        ? 'bg-gradient-to-br from-purple-500 to-indigo-500 text-white shadow-lg shadow-indigo-500/40 animate-pulse'
                        : 'text-gray-300 hover:text-white hover:bg-white/10'
                    }`}
                >
                    {isUpscaleTab && <ExpandIcon className="w-5 h-5 mr-2" />}
                    {tab}
                </button>
                );
            })}
